...
$(function(){
	//スワップイメージ
	$.fn.swap({
		box : '.wcswap', //全体をくくるdivのクラス
		l: '.wcswap_l', //大きい画像が入るdivのクラス
		s: '.wcswap_s', //小さい画像が入るdivのクラス
		action: 'click' //「click」ならクリック、「hover」ならホバー
	});

	//ロールオーバー
	$.fn.btn({
		classname: 'Hover', //ロールオーバーするクラス（imgに付加する）
		ext: '_over' //入れ替えるファイル名（例：sample_on.jpg）
	});
	
	//Lightbox for jQuery
	$('.lightbox') //Lightboxを実現する画像に付加するクラス名
	.parent('[href$="jpg"],[href$="jpeg"],[href$="gif"],[href$="png"],[href$="JPG"],[href$="GIF"],[href$="PNG"]').not('[href^="http"],[href^="https"]').lightBox({
		...
		overlayBgColor: '#000' //クリック後の背景の色（変更すれば黒以外も可能）
	});
